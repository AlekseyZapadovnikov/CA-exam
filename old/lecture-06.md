...
Трудности эксплуатации аппаратного обеспечения связаны с тем, что аппаратура живёт в физическом мире.
Она изнашивается, ломается и требует обслуживания.
Амортизация означает деградацию надёжности и выход оборудования из строя со временем.
Физический доступ нужен для ремонта, замены и иногда обновления.
Но доступ может быть дорогим, опасным или невозможным.
Среда эксплуатации тоже влияет на работу: питание, температура, помехи, механические воздействия и долгосрочные эффекты.
Например, удалённое устройство или космический аппарат нельзя просто открыть и перепаять.
Поэтому эксплуатация аппаратуры всегда включает риски, которых почти нет у чистого ПО.

### Схема / картинка

![Восстановление аппаратуры](../assets/course-fig/hardware-recovery-1.png)

Картинка из блока про отладку и восстановление показывает, что ремонт аппаратуры часто требует физической работы с изделием.
...
Проблему устаревающей аппаратуры можно решать заменой железа, сохранением интерфейса или переносом поведения в ПО.
Первый подход — перепроектировать устройство на новой элементной базе.
Второй подход — сделать модульную замену, которая выглядит для старой системы как прежнее устройство.
Третий подход — имитировать старую систему через стандартные интерфейсы.
Четвёртый подход — виртуализировать старую платформу.
С точки зрения аппаратуры важно сохранить электрические и логические интерфейсы.
С точки зрения ПО важно, чтобы старая программа продолжала видеть привычное окружение.
Например, современное устройство может притворяться старым накопителем, чтобы старое ПО работало без переписывания.
...
Сборка и комплектация выбирают нужные модули, платы расширения и перемычки.
Реконфигурация меняет связи между вычислительными узлами после производства.
Программирование задаёт поведение данными или инструкциями в памяти.
Например, один и тот же процессор становится разным инструментом в зависимости от программы.

### Схема / картинка

![2-этапное производство](../assets/course-fig/platform-based-design-only-watch.png)

Схема показывает базовую платформу, конфигурацию прикладного поведения и экземпляр сконфигурированной системы.
...
**Сборка и комплектация** работают на уровне состава изделия.
...
Hardware — то, что тяжело, долго или дорого поменять.
Software — то, что легко, быстро или дёшево поменять.
Например, ПЛИС программируется, но работает как схема, поэтому в использовании часто воспринимается как аппаратура.
...
Программное обеспечение открывает быстрый цикл разработки.
Его можно собрать, проверить, заменить и обновить быстрее, чем аппаратное изделие.
ПО легко доставлять пользователю, в том числе удалённо.
Это даёт гибкость: одну и ту же аппаратуру можно приспособить к разным задачам.
Но гибкость создаёт и риски: пользователь может стать beta-тестером.
Удалённые обновления могут происходить даже без информированного согласия.
Автоматизация CI/CD ускоряет создание и внедрение ПО.
Цена этой свободы — высокая сложность программ и рост контроля поставщика над системой.

### Схема / картинка

![Gitflow и обновления ПО](../assets/course-fig/gitflow.png)

Схема иллюстрирует, что современная разработка ПО строится вокруг автоматизированного процесса изменений.
...
CoDesign проектирует систему целиком и дольше держит границу между HW и SW открытой.
В CoDesign разрабатывают интерфейс HW/SW, модели, получают обратную связь, затем заменяют модели реальными HW/SW.
...
![Hardware/Software CoDesign](../assets/course-fig/hs-sw-co-design.jpg)

Схема показывает совместную разработку аппаратной и программной частей через модели и интерфейс.
...
![Модель вычислений](../assets/course-fig/moc.png)
